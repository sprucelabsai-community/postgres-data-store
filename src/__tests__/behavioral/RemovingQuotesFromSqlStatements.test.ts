...
export default class RemovingQuotesFromSqlStatementsTest extends AbstractSpruceFixtureTest {
	...
	protected static async canDisableQuotesFromUpdate() {
		const { sql } = this.query.update('test', { id: '123' }, { name: 'test' })
		assert.isEqual(sql, 'UPDATE test SET name = $1 WHERE id = $2 RETURNING *')
	}
